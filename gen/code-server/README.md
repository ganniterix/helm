# Helm chart (V3) - code-server

## code-server

[Code-server](https://coder.com) is VS Code running on a remote server, accessible through the browser.

The the docker image is part of the the [`linuxserver.io`](https://www.linuxserver.io/) collection, it has a great [documentation](https://github.com/linuxserver/docker-code-server) that cover all the image features.
...
```bash
helm install code-server code-server
```

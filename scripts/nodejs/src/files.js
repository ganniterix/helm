...
const mapFile =
  ...
  ({ fileName, fullPath, content }) => {
    ...
    const description = lines.find((line) =>
      ...
      line
        .toLowerCase()
        .includes(`[${config.files.descriptionsMap[name] || name}]`)
    );
    ...
    const yamlMatch = /```yaml[\s\S]*?services:\s(?<content>[\s\S]*?)```/g.exec(
      content
    );
